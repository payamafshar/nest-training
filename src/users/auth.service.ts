...
export default class AuthService {
  ...
  async getOtpfromUser(createOtpDtop: CreateOtpDtop) {
    ...
    if (!user) {
      ...
      const updatedOtp = await this.repo.update(
        { mobile },
        { otpCode, otpExpiresIn },
      );
  ...
  async checkOtpFromUser(checkOtpDto: CheckOtpDto) {
    ...
    const token = jwt.sign({ mobile: user.mobile }, 'dontguesssecret', {
      expiresIn: '24days',
    });
