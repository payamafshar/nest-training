...
export class ProductService {
  ...
  async create(
    ...
    const category = await this.categoryRepo.findOne({
      where: { englishTitle: categoryTitle },
    });
  ...
  async createBasket(productId: number, user: UserEntity) {
    ...
    const findedBasket = await this.basketRepo.findOne({
      where: { loggedUserId: loggedInUser },
      relations: ['user', 'orders', 'orders.product'],
    });

    const createOrder = this.orderRepo.create({
      quantity: 1,
      logginUserId: loggedInUser,
    });
    ...
    if (!findedBasket) {
      ...
      const productBasket = this.basketRepo.create({
        totalCount,
        totalPayment,
      });
      ...
      findedBasket.totalPayment = findedBasket.orders.reduce(
        (total: number, item: OrderEntity): any => {
          return total + item.quantity * +item.product.price;
        },
        0,
      );
      findedBasket.totalCount = findedBasket.orders.reduce(
        (total: number, item: OrderEntity): any => {
  ...
  async increaseProductInBasket(productId: number, user: UserEntity) {
    ...
    const findProductInBasket = findedBasket.orders.find(
  ...
  async decreaseProductInBasket(productId: number, user: UserEntity) {
    ...
    if (findProductInBasket.quantity > 1) {
      ...
      const filterdData = findedBasket.orders.filter(
        (item) => item.product.id != productId,
      );
  ...
  async deleteProductInBasket(productId: number, user: UserEntity) {
    ...
    const deletedProductArray = findedBasket.orders.filter(
      (item) => item.product.id != productId,
    );
